async function init(depth = 2) {
	...
	const appContext = new ContextContainer(DIConfig.slice(0, depth), "appContextInit" + depth);
	appContext.init();

	return {
		suiteName: "ContextContainer.init() with depth = " + depth,
		fn: () => {
			appContext.init();
		},
	};
